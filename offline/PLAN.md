...
- **Tap a wedge = pick that area** (no spin). **SPIN** picks the winner uniformly first, animates the wheel to it, then picks that area. Either way a 3-second **Get Ready** screen (5 s at first, cut to 3 s in QA) (`TIMING.readyMs`) runs before the first question and cannot be cut short (Ben rejected a Skip button; Escape still returns to the wheel). *Added 2026-09-17 after QA on a real machine; the first cut had dropped it.*
- **Dropped** from the old flow: difficulty slider, loading screens, `/api/results` AI feedback (replaced by score-band static copy), the footer link wall (a one-line footer was kept at first, then removed in QA).
