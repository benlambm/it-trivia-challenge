...
> 25 questions. 5 IT categories. One AI guidance counselor who decides if you're a Help Desk Hero or a Cyber Rookie.

**Live:** [trivia.benlamb.net](https://trivia.benlamb.net)

This is a small React app that plays a multiple-choice trivia game. The twist: every question and every results screen is **generated on the fly by Google Gemini**. There's no question bank. There's no hand-written feedback. You hit "Start," the backend asks Gemini for 25 fresh questions about Networking, AI, Programming, Cybersecurity, and IT Ops, and the game plays them. When you finish, Gemini reads your score and writes you a custom title + motivational message.
...
You'll need:
- **Node 20+** (24 LTS is what production runs on)
...
You'll need a way for the frontend dev server to talk to the backend on a different port. The easiest path: add a Vite proxy. In `web/vite.config.ts`:
...
Now `fetch('/api/questions')` from the React app goes through Vite to your local Node backend, which calls Gemini, which streams back JSON. Open the Network tab and watch it happen — it's a satisfying loop.
...
- Write tests! There aren't any yet. There should be.
